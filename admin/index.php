...
function isPointInPolygon($pointsCount, $xCoords, $yCoords, $longitude, $latitude)
{
    $i = $j = $c = 0;
    for ($i = 0, $j = $pointsCount ; $i < $pointsCount; $j = $i++) {
        $point = $i;
        if ( (($yCoords[$point] >  $latitude !== ($yCoords[$j] > $latitude)) &&
            ($longitude < ($xCoords[$j] - $xCoords[$point]) * ($latitude - $yCoords[$point]) / ($yCoords[$j] - $yCoords[$point]) + $xCoords[$point]) ) ) {
            $c = !$c;
        }
    }
...
foreach ($json['features'] as $borough) {
    ...
    $boroughs[$props['NOM']] = ['coords' => $borough['geometry']['coordinates']];

    $boroughs[$props['NOM']]['x'] = array_column($borough['geometry']['coordinates'][0][0], 1);
    $boroughs[$props['NOM']]['y'] = array_column($borough['geometry']['coordinates'][0][0], 0);
}

function getBoroughName($coords) {
    list($lat, $long) = explode(",", $coords);
    global $boroughs;
    foreach ($boroughs as $name => $borough) {

        $points = count($borough['x']);
        if (isPointInPolygon($points, $borough['x'], $borough['y'], (float)$long, (float)$lat)) {
            return $name;
        }
    }
}
